...
void AC_E_Cut_2D(Int_t run_number=5, Int_t flag=0, Int_t QUIET=0) { 
...
Int_t adc_ACU[12];		Int_t tdc_ACU[12][16];
Int_t adc_ACD[12];		Int_t tdc_ACD[12][16];
...
TH2D *h_ADC_TDC_ac[24];			 char Title_ADC_TDC_ac[24][100]; char Name_ADC_TDC_ac[24][100];
...
for(int i=0; i<12; i++){
	...
	sprintf(Title_TDC_TOF1_diff, "Run %d  |  TDC_TOF1_UP - TDC_TOF1_Down", run_number);
	h_TDC_TOF1_diff[i] = new TH1D(Name_TDC_TOF1_diff, Title_TDC_TOF1_diff, 400, -400, 400);
}
...
for (int i=0; i<24; i++) {
	...
	sprintf(Title_ADC_TDC_ac[i],"ADC vs. TDC (Ch. %d)  --  AC",i);
	sprintf(Name_ADC_TDC_ac[i],"ADC vs. TDC (Ch. %d) - AC",i);
}
...
h_ADC_TDC_ac[i] = new TH2D(Name_ADC_TDC_ac[i], Title_ADC_TDC_ac[i],256,0,4000,128,0,4000);
}
...
//for (Int_t ievt=0; ievt<10000; ievt++) {
...
//    		cout << ievt << "  " << "TEST" << endl;
	...
	if ((good_TOF) && (good_MWPC)) {
		...
		if(TDC_TOF1U[gap_to_fit-1]!=-1 || TDC_TOF1D[gap_to_fit-1]!=-1){
			...
			h_TDC_TOF1_diff[gap_to_fit-1]->Fill(TDC_diff[gap_to_fit-1]);
		}
		...
		for(int q=0; q<12; q++){
			h_ADC_TDC_ac[q]->Fill(tdc_ACU[q][0],adc_ACU[q]);
			h_ADC_TDC_ac[q+12]->Fill(tdc_ACD[q][0],adc_ACD[q]);
		}
...
char Name_Can_ADC_TDC_AC[100];			char Title_Can_ADC_TDC_AC[100];

sprintf(Name_Can_ADC_TDC_AC,"AC TDC Distributions -- Run %d  (Ch. 0 - 23) -- Mapping Sorted",run_number);
sprintf(Title_Can_ADC_TDC_AC,"AC TDC Distributions -- Run %d  (Ch. 0 - 23) -- Mapping Sorted",run_number);

TCanvas *cc1;
	cc1 = new TCanvas(Name_Can_ADC_TDC_AC,Title_Can_ADC_TDC_AC,1200,600); 
	cc1->Divide(6,4);
	...
	for(Int_t ican=0; ican<24; ican++){
		cc1->cd(ican+1);
		//h_ADC_TDC_ac[ican]->SetAxisRange(TOF2_xmin, TOF2_xmax, "X");
		h_ADC_TDC_ac[ican]->Draw("colz");
	}
