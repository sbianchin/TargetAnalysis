...
void Test(Int_t run_number=5, Int_t flag=0, Int_t nevt=0) { 
	...
	if(flag==7){		
		...
		for (int i=0; i<12; i++){
			sprintf(Title_ADC_tof1U[i],"Raw ADC (V792 Ch. %d)  |  %d #leq TDC #leq %d  --  TOF1 (%d UP)",TOF1U_ADC[i],i+1,TOF1U_TDC_min[i],TOF1U_TDC_max[i]);
			sprintf(Name_ADC_tof1U[i],"ADC_TOF1 (%d UP)",i+1);
			sprintf(Title_ADC_tof1D[i],"Raw ADC (V792 Ch. %d)  |  %d #leq TDC #leq %d  --  TOF1 (%d DOWN)",TOF1D_ADC[i],i+1,TOF1D_TDC_min[i],TOF1D_TDC_max[i]);
			...
			sprintf(Title_TDC_tof1U[i],"Raw TDC (HR_TDC Ch. %d)  |  %d #leq TDC #leq %d  --  TOF1 (%d UP)",TOF1U_TDC[i],i+1,TOF1U_TDC_min[i],TOF1U_TDC_max[i]);
			sprintf(Name_TDC_tof1U[i],"TDC_TOF1 (%d UP)",i+1);
			sprintf(Title_TDC_tof1D[i],"Raw TDC (HR _TDC Ch. %d)  |  %d #leq TDC #leq %d  --  TOF1 (%d DOWN)",TOF1D_TDC[i],i+1,TOF1D_TDC_min[i],TOF1D_TDC_max[i]);
	...
	if(flag==8){
		for(int i=0; i<12; i++){
			sprintf(Title_ADC_tof2AO[i],"Raw ADC (V792 Ch. %d)  --  TOF2 (%d A OUT)",TOF2AO_ADC[i],i+1);
			sprintf(Name_ADC_tof2AO[i],"ADC_TOF2 (%d AO)",i+1);
			sprintf(Title_ADC_tof2AI[i],"Raw ADC (V792 Ch. %d)  --  TOF2 (%d A IN)",TOF2AI_ADC[i],i+1);
			sprintf(Name_ADC_tof2AI[i],"ADC_TOF2 (%d AI)",i+1);
			sprintf(Title_ADC_tof2BO[i],"Raw ADC (V792 Ch. %d)  --  TOF2 (%d B OUT)",TOF2BO_ADC[i],i+1);
			sprintf(Name_ADC_tof2BO[i],"ADC_TOF2 (%d BO)",i+1);
			sprintf(Title_ADC_tof2BI[i],"Raw ADC (V792 Ch. %d)  --  TOF2 (%d B IN)",TOF2BI_ADC[i],i+1);
			sprintf(Name_ADC_tof2BI[i],"ADC_TOF2 (%d BI)",i+1);

			sprintf(Title_TDC_tof2AO[i],"Raw TDC (HR_TDC Ch. %d)  --  TOF2 (%d A OUT)",TOF2AO_TDC[i],i+1);
			sprintf(Name_TDC_tof2AO[i],"TDC_TOF2 (%d AO)",i+1);
			sprintf(Title_TDC_tof2AI[i],"Raw TDC (HR_TDC Ch. %d)  --  TOF2 (%d A IN)",TOF2AI_TDC[i],i+1);
			sprintf(Name_TDC_tof2AI[i],"TDC_TOF2 (%d AI)",i+1);
			sprintf(Title_TDC_tof2BO[i],"Raw TDC (HR_TDC Ch. %d)  --  TOF2 (%d B OUT)",TOF2BO_TDC[i],i+1);
			sprintf(Name_TDC_tof2BO[i],"TDC_TOF2 (%d BO)",i+1);
			sprintf(Title_TDC_tof2BI[i],"Raw TDC (HR_TDC Ch. %d)  --  TOF2 (%d B IN)",TOF2BI_TDC[i],i+1);
