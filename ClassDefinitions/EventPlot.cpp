 ...
 *   canvas->Divide(3,2);
 *   EventPlot* event_plot = new EventPlot("Example Plot");
...
TGraphErrors *EventPlot::ShowBars(std::vector<int> bars, Color_t color) {
    ...
    for (int bar : bars) {
        coords_x.push_back(DG::Xloc[bar]);
        coords_y.push_back(DG::Yloc[bar]);
        errors_x.push_back(DG::TARGET_Errors_X);
        errors_y.push_back(DG::TARGET_Errors_Y);
    }
...
void EventPlot::Draw(TCanvas *canvas, unsigned int subplot) {
    ...
    TLine *gaps[12] = {DG::Gap1l, DG::Gap2l, DG::Gap3l, DG::Gap4l, DG::Gap5l, DG::Gap6l, 
                       DG::Gap7l, DG::Gap8l, DG::Gap9l, DG::Gap10l, DG::Gap11l, DG::Gap12l};
    for (TLine *gap : gaps) {
        gap->SetLineWidth(10);
        gap->SetLineColor(15);
    }
...
void EventPlot::Frame(TCanvas *canvas, unsigned int subplot) {
    canvas->cd(subplot)->Range(-50, -50, 50, 50);
    ...
    // const float R_TOF1 = 47.1;
    // TEllipse *ell_TOF1 = new TEllipse(0, 0, R_TOF1, 0);
    // ell_TOF1->SetFillStyle(0);
    // ell_TOF1->SetLineColor(kMagenta);
    // ell_TOF1->SetLineWidth(1);
    // ell_TOF1->Draw();
    const float R_TARGET = 29.0;
    TEllipse *ell_TARGET = new TEllipse(0, 0, R_TARGET, 0);
    ell_TARGET->SetFillStyle(0);
    ell_TARGET->SetLineColor(kBlack);
...
void EventPlot::DrawTOF1(TCanvas *canvas, unsigned int subplot) {
    ...
    TEllipse *ell_TOF1 = new TEllipse(0, 0, CP::R_TOF1, 0);
    ell_TOF1->SetFillStyle(0);
    ell_TOF1->SetLineColor(6);
    ell_TOF1->SetLineWidth(1);
    ell_TOF1->Draw();
}

void EventPlot::DrawSFT_L1(TCanvas *canvas, unsigned int subplot){

    canvas->cd(subplot)->Range(-50, -50, 50, 50);
    
    TEllipse *ell_SFT_L1 = new TEllipse(0, 0, CP::R_SFT_L1, 0);
    ell_SFT_L1->SetFillStyle(0);
    ell_SFT_L1->SetLineColor(kBlue);
    ell_SFT_L1->SetLineWidth(1);
    ell_SFT_L1->Draw();
}
