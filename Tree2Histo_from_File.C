...
void Tree2Histo(Int_t run_number=5, Int_t flag=0, Int_t QUIET=0) { 
	...
	char file_input[100] = "kekcc_all_7_evt.txt";
	//char file_input[100] = "kekcc_all_8_evt.txt";
	//char file_input[100] = "kekcc_all_merged_78_evt.txt";
    ifstream fdat_read(file_input, ios::in); 
    int Read_event;
    int counter = 0;
	...
	sprintf(Name_foutput,"%s/Hist_Run%dM_from_File.root",path_output, run_number);
	...
	while(i<nentries && fdat_read.good()){
		fdat_read >> Read_event;
		i=Read_event;
		counter++;
		if(counter%10000==0 && i>0) cout << counter << " events processed!" << endl;
		//cout << Read_event << "   " << i << endl;
